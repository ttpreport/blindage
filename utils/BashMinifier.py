# Original: https://github.com/oliv5/bash_minifier/blob/master/minifier.py
...
class BashFileIterator:
    ...
    def pushDelimiter(self, character, _type=''):
        ...
        last = self._delimiters_stack[-1] if len(
            self._delimiters_stack) > 0 else BashFileIterator._Delimiter('')
...
def minify(src):
    ...
    for ch in it.charactersGenerator():
        if it.isInsideAnything() or ch != "\n":
            ...
            if it.getNextCharacter() == '{' and it.getPreviousCharacters(2) == "()":
    ...
    # characters that may not be surrounded by whitespaces
    other_delimiters = ('|', '&', ';', '<', '>', '(')
    for ch in it.charactersGenerator():
        if it.isInsideGroupWhereWhitespacesCannotBeTruncated():
            src += ch
        elif ch in (' ', '\t') \
                and (it.getPreviousCharacter() in other_delimiters
                     or it.getNextCharacter() in other_delimiters
                     or it.getPreviousCharacters(2) in ('()',)) \
